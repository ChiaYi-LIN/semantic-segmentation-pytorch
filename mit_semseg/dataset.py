...
class TrainDatasetSquareCrop(BaseDataset):
    def __init__(self, root_dataset, odgt, opt, batch_per_gpu=1, **kwargs):
        super(TrainDatasetSquareCrop, self).__init__(odgt, opt, **kwargs)
        self.root_dataset = root_dataset
        self.batch_per_gpu = batch_per_gpu
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.imgSizes, list) or isinstance(self.imgSizes, tuple):
            ...
            this_short_size = self.imgSizes
        this_short_size = math.floor(this_short_size / 64) * 64
        ...
        for i in range(self.batch_per_gpu):
            ...
            batch_widths[i] = math.ceil(img_width * this_scale)
            batch_heights[i] = math.ceil(img_height * this_scale)

        batch_images = torch.zeros(
            self.batch_per_gpu, 3, this_short_size, this_short_size)
        batch_segms = torch.zeros(
            self.batch_per_gpu, this_short_size, this_short_size).long()
        ...
        for i in range(self.batch_per_gpu):
            ...
            img, segm = randomCrop(img, segm, this_short_size, this_short_size)
